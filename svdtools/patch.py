...
class Device:
    ...
    def process_peripheral(self, pspec, peripheral, update_fields=True):
        ...
        for ptag in self.iter_peripherals(pspec, check_derived=False):
            ...
            for cspec in peripheral.get("_clustermatch", {}):
                for ctag in p.iter_clusters(cspec):
                    c = Cluster(ptag, ctag)
                    cmod = peripheral["_clustermatch"][cspec]

                    # Handle deletions
                    deletions = cmod.get("_delete", [])
                    if isinstance(deletions, list):
                        for rspec in deletions:
                        ...
                        for rspec in deletions:
                            if rspec == "_registers":
                                ...
                                c.delete_register(rspec)
                    # Handle modifications
                    for rspec in cmod.get("_modify", {}):
                        rmod = cmod["_modify"][rspec]
                        if rspec == "_registers":
                            for rspec in rmod:
                                c.modify_register(rspec, rmod[rspec])
                        else:
                            c.modify_register(rspec, rmod)
                    # Handle additions
                    for rname in cmod.get("_add", {}):
                        radd = cmod["_add"][rname]
                        if rname == "_registers":
                            for rname in radd:
                                c.add_register(rname, radd[rname])
                        else:
                            c.add_register(rname, radd)
                    for rname in cmod.get("_derive", {}):
                        rderive = cmod["_derive"][rname]
                        if rname == "_registers":
                            for rname in rderive:
                                c.derive_register(rname, rderive[rname])
                        else:
                            c.derive_register(rname, rderive)
                    # Handle registers
                    for rspec in cmod:
                        if not rspec.startswith("_"):
                            register = cmod[rspec]
                            c.process_register(rspec, register, update_fields)
        if pcount == 0:
            raise MissingPeripheralError("Could not find {}".format(pspec))


class Cluster:
    """Class collecting methods for processing clusters of registers"""
    ...
    def iter_registers(self, rspec):
        """Iterates over all registers that match rspec and live inside ctag."""
    ...
    def modify_register(self, rspec, rmod):
        """Modify rspec inside ctag according to rmod."""
    ...
    def add_register(self, rname, radd):
        """Add rname given by radd to ctag."""
        for rtag in self.ctag.iter("register"):
            if rtag.find("name").text == rname:
                raise SvdPatchError(
                    "peripheral {} cluster {} already has a register {}".format(
                        self.ptag.find("name").text, self.ctag.find("name").text, rname
                    )
                )
        rnew = ET.SubElement(self.ctag, "register")
    ...
    def derive_register(self, rname, rderive):
        """Add rname given by deriving from rsource to ctag"""
        ...
        if source == None:
            raise SvdPatchError(
                "peripheral {} cluster {} does not have register {}".format(
                    self.ptag.find("name").text, self.ctag.find("name").text, srcname
                )
            )
    ...
    def process_register(self, rspec, register, update_fields=True):
        ...
        cname = self.ctag.find("name").text
        rcount = 0
        for rtag in self.iter_registers(rspec):
            r = Register(rtag)
            rcount += 1
        ...
        if rcount == 0:
            raise MissingRegisterError("Could not find {}:{}:{}".format(pname, cname, rspec))
